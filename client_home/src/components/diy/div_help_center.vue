			...
			<div class="container">
				...
				<div class="row">
					...
					<div class="details_btn_wrap">
								<button class="btn btn-primary details_btn" v-if="$check_action('/help_center/details','set')" @click="change_praise()" >点赞</button>
								<button class="btn btn-primary details_btn" v-if="$check_action('/help_center/details','set')" @click="change_collect()" >收藏</button>
							</div>
						<!-- 富文本 -->
...
<script>
	...
	export default {
      ...
      check_collect() {
        var user_id = this.$store.state.user.user_id;
        var help_center_id = this.obj.help_center_id;
        this.$get('~/api/collect/count?', {
          user_id,
          source_table: "help_center",
          source_field: "help_center_id",
          source_id: help_center_id
        }, (res) => {
