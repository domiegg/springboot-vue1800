<template>
	<div class="diy_details page_help_center" id="help_center_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_help_center'>
							<div_help_center :obj="obj"></div_help_center>
...
<script>
	import div_help_center from "@/components/diy/div_help_center.vue";
	...
	export default {
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/help_center/set?help_center_id=' + obj["help_center_id"], {
					hits
				}, (res) => {
					if(res.result){
						...
						var body = {
							source_table: "help_center",
							source_field: "help_center_id",
							source_id: this.obj.help_center_id,
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.help_center_id;
				}
			},
			/**
			 * 初始化收藏状态
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					this.state_collect = true;
										query.img = this.obj.help_cover
										query.title = this.obj.help_topics
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/help_center/set?help_center_id=' + _this.obj["help_center_id"], {
								praise_len
							}, (res) => {
								if(res.result){
