                ...
                <el-form-item label="分类" prop="cart_type">
                    <el-select v-model="form.cart_type">
                        <el-option v-for="(o ,i) in list_cart_goods_type" :value="o.name" :key="i"
							...
							<el-col v-if="user_group === '管理员' || $check_field('get','article_no') || $check_field('add','article_no') || $check_field('set','article_no')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="商品编号" prop="article_no">
												<el-input id="article_no" v-model="form['article_no']" placeholder="请输入商品编号"
							  v-if="user_group === '管理员' || (form['mall_center_id'] && $check_field('set','article_no')) || (!form['mall_center_id'] && $check_field('add','article_no'))" :disabled="disabledObj['article_no_isDisabled']"></el-input>
					<div v-else-if="$check_field('get','article_no')">{{form['article_no']}}</div>
											</el-form-item>
			</el-col>
								<el-col v-if="user_group === '管理员' || $check_field('get','sales_type') || $check_field('add','sales_type') || $check_field('set','sales_type')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="销售类型" prop="sales_type">
												<el-input id="sales_type" v-model="form['sales_type']" placeholder="请输入销售类型"
							  v-if="user_group === '管理员' || (form['mall_center_id'] && $check_field('set','sales_type')) || (!form['mall_center_id'] && $check_field('add','sales_type'))" :disabled="disabledObj['sales_type_isDisabled']"></el-input>
	...
	export default {
		...
		data() {
			return {
				...
				disabledObj:{
								"article_no_isDisabled": false,
										"sales_type_isDisabled": false,
										},
		...
		methods: {
            ...
            submit_goods(source_id,add_flag){
				let customize_field = []
							customize_field.push({"field_name":"商品编号","field_value":this.form.article_no});
									customize_field.push({"field_name":"销售类型","field_value":this.form.sales_type});
			                this.goods_form = {
                    goods_id: 0,
