	...
	<el-main class="bg table_wrap">
		<el-form label-position="right" :model="query" class="form p_4" label-width="120">
			<el-row>
					...
					<el-form-item label="帮助主题">
									<el-input v-model="query.help_topics"></el-input>
								</el-form-item>
				</el-col>
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
			<el-table-column fixed type="selection" tooltip-effect="dark" width="55">
				...
				<el-table-column prop="help_cover" @sort-change="$sortChange" label="帮助封面"
				v-if="user_group == '管理员' || $check_field('get','help_cover')" min-width="200">
						<template slot-scope="scope">
					<el-image style="width: 100px; height: 100px" :src="$fullUrl(scope.row['help_cover'])"
						:preview-src-list="[$fullUrl(scope.row['help_cover'])]">
					...
					<el-table-column prop="help_topics" @sort-change="$sortChange" label="帮助主题"
				v-if="user_group == '管理员' || $check_field('get','help_topics')" min-width="200">
					</el-table-column>
			...
			<el-table-column fixed="right" label="操作" min-width="120" v-if="user_group == '管理员' || $check_action('/help_center/table','set') || $check_action('/help_center/view','set') || $check_action('/help_center/view','get') || $check_action('/${table.tableConfig.table_nav_name}/table','add') || $check_action('/${table.tableConfig.table_nav_name}/view','add')" >
				...
				<template slot-scope="scope">
					...
					v-if="user_group == '管理员' || $check_action('/help_center/table','set') || $check_action('/help_center/view','set') || $check_action('/help_center/view','get')"
						...
						<router-link v-if="user_group == '管理员' || $check_comment('/help_center/table')" class="el-button el-button--small is-plain el-button--primary"
								  :to="'../comment/table?size=10&page=1&source_table=help_center&source_field=' + field + '&source_id=' + scope.row[field]" size="small">
					查看评论
					</router-link>
					</template>
			</el-table-column>
