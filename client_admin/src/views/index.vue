					...
					<el-col :span="8">
						<div class="card chart" >
							...
							<div v-if="!list_goods_price.length">订单没有符合条件的数据</div>
												...
												<el-col v-if="user_group == '管理员' || $check_figure('/financial_management/table')" :span="8">
						<div class="card chart">
									<newBarChart v-if="bar_obj_financial_management.values.length > 0" id="bar_obj_financial_management" :vm="bar_obj_financial_management" :title="'财务管理统计'">
							</newBarChart>
							<div v-if="!bar_obj_financial_management.values.length">财务管理没有符合条件的数据</div>
								</div>
					</el-col>
					</el-row>
...
<script>
	...
	export default {
		...
		created() {
										// 执行财务管理数据获取
			this.get_list_financial_management();
								...
								(json) => {
									if (json.result) {
										list[i].name = json.result.obj.nickname;
																...
																// 获取财务管理统计柱状图
			async get_list_financial_management() {
				let name_list = [];
				let query_str = "";
									let group_by_value = "month";
								let flag = false;
												let date_flag = "其他"
																						name_list.push("销售额");
				query_str = query_str+"sales_volume"+","
																																							name_list.push("利润");
				query_str = query_str+"profit"+","
									this.bar_obj_financial_management.names = name_list
				query_str = query_str.substr(0,query_str.length-1);
				let data = {};
						await this.$get(
						"~/api/financial_management/bar_group?field="+query_str+"&groupby="+group_by_value,
						data,
						(json) => {
							if (json.result) {
								let xAxis = [];
								...
								this.bar_obj_financial_management.xAxis = xAxis;
								this.bar_obj_financial_management.values = values;
							}
							if (flag){
								this.get_nickname(this.bar_obj_financial_management.xAxis,true);
							}
						});
			},
